...
int largoTotal = 2000;  //se mide en pasos de motor
int pasosxseg = 0; 
int contapasos= 0;
int tiempo1 = 0;
int tiempo2 = 0;
...
void menuAction(unsigned int menuCode) {
 ...
 Serial.println(menuCode);
      ...
      switch (menuCode) {
        ...
        while(val == 1 && con == 0) // el con es mi flag, cuando toque el Final de carrera val sera 1 y con se pone a 1,
                                    //asì cuando vuelve a pasar por el while, con nunca mas será 0.
        {  
        stepper.rotate(pasos); //arranca girando el motor hacia la izquierda en busca del FC.
        val = digitalRead(4); //leo el FC
        delay(1);           //Antirebote
        val = digitalRead(4); //leo el FC
